...
public class AddProducToShopsUseCase extends UseCase<ProductDataModel, Boolean> {

    private static final String TAG = AddProducToShopsUseCase.class.getSimpleName();
    private AddProductRepository addProductRepository;

    @Inject
    protected AddProducToShopsUseCase(UseCaseComposer useCaseComposer, AddProductRepository addProductRepository) {
    ...
    protected Observable<Boolean> createUseCaseObservable(ProductDataModel productDataModel, Context context) {
        ...
        return addProductRepository.addProductToShops(productDataModel, SharedPrefManager.get_userEmail());
    }
}
