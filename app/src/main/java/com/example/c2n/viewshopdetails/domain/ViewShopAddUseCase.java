...
public class ViewShopAddUseCase extends UseCase<ShopDataModel, DocumentReference> {

    ViewShopRepository viewShopRepository;
    DocumentToShopDataModel documentToShopDataModel;

    @Inject
    protected ViewShopAddUseCase(UseCaseComposer useCaseComposer, ViewShopRepository viewShopRepository, DocumentToShopDataModel documentToShopDataModel) {
...
//        return viewShopRepository.getShopDetails(shopDataModel).map(documentSnapshot -> documentToShopDataModel.mapDocumentToShopDataModel(documentSnapshot));
        return viewShopRepository.addShop(shopDataModel);
    }
}
